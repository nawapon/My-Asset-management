...
app.post('/api/repairs', authenticateToken, asyncHandler(async (req, res) => {
    ...
    const params = [equipment.id, req.user.id, req.user.fullName, reporterLocation, reporterContact, problemDescription, new Date(), 'Pending'];
...
app.post('/api/equipment/import', authenticateToken, upload.single('csvFile'), (req, res) => {
    // ... (This route has side effects and complex flow, keeping it with callbacks is fine)
});
