...
const { sendRepairNotification } = require('./telegram.js');
...
app.put('/api/repairs/:id', authenticateToken, asyncHandler(async (req, res) => {
    ...
    // Check status and update corresponding date fields
    if (status === 'In Progress') {
        // Only set acceptedDate if it's currently NULL
        updateFields.push('acceptedDate = COALESCE(acceptedDate, ?)');
        queryParams.push(now);
    } else if (status === 'Completed') {
        // Always set completedDate when status is 'Completed'
